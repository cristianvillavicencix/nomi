...
**I have a *Security Definer View* error in *Security Advisor* for `init_state`**

This was addressed in migration `20260830140000_init_state_security_invoker.sql`: the view now uses `security_invoker = true` and delegates the privileged count to `public.crm_is_initialized()` (a narrow `SECURITY DEFINER` function). After applying migrations, re-run Security Advisor to confirm the view warning is cleared.

**Security Advisor phase 1 (search_path + internal RPC lockdown)**

Migration `20260831120000_security_advisor_phase1.sql` pins `search_path` on functions flagged by the linter, revokes direct browser access to cron/secret helpers (service role only), and blocks anonymous RPC on app helpers while keeping authenticated grants used by RLS and the CRM.

**Security Advisor phase 2 (private storage buckets)**

Migration `20260831140000_storage_buckets_phase2_private.sql` sets `attachments`, `avatars`, and `form-branding` buckets to **private** (no public listing). The app and edge functions use **signed URLs** at read time via `src/lib/supabase/storageObjectUrl.ts` and `supabase/functions/_shared/storageObjectUrl.ts`. Existing rows that store legacy `/object/public/...` URLs remain compatible because parsers extract the bucket/path and sign on demand.

**Remaining `authenticated_security_definer_function_executable` warnings**

The CRM exposes `SECURITY DEFINER` helpers (`can_view_deal`, `current_user_org_id`, etc.) to the `authenticated` role for RLS and RPC. Supabase Security Advisor will keep flagging those while they stay in `public` and callable via PostgREST — that is expected and safe when the function body enforces org/member checks. Migration `20260831150000_security_advisor_revoke_public_rpc.sql` removes **anonymous** RPC access (`PUBLIC` + `anon`) except `crm_is_initialized()` for the setup gate.

**Leaked password protection (Dashboard, not SQL)**

In **Authentication → Providers → Email**, enable **Prevent use of leaked passwords** (Have I Been Pwned check). This clears the `auth_leaked_password_protection` Security Advisor warning and blocks compromised passwords at sign-up/password change.
