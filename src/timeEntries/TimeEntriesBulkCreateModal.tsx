...
const dayTypeTooltipText = (
  dayType: DayDraft["day_type"] | undefined,
): string => {
...
const normalizeTimeForInput = (value?: string | null) => {
  ...
  if (typeof value === "number" && Number.isFinite(value)) {
    const totalMinutes =
      value > 1440 ? Math.round(value / 60) : Math.round(value);
...
const normalizeProjectId = (
  value?: TimeEntry["project_id"] | DayDraft["project_id"],
...
const existingEntryComparableKey = (entry: TimeEntry) => {
  ...
  return JSON.stringify({
    ...
    break_minutes: Number(
      entry.lunch_minutes ?? entry.break_minutes ?? meta.lunch_minutes ?? 0,
    ),
...
const payloadComparableKey = (payload: Omit<TimeEntry, "id">) => {
  ...
  return JSON.stringify({
    ...
    break_minutes: Number(
      payload.lunch_minutes ?? payload.break_minutes ?? meta.lunch_minutes ?? 0,
    ),
...
export const TimeEntriesBulkCreateModal = ({ open, onOpenChange }: Props) => {
  ...
  const [previewEmployeeId, setPreviewEmployeeId] = useState<number | null>(
    null,
  );
  const [activeAddressRowId, setActiveAddressRowId] = useState<string | null>(
    null,
  );
  const [googleAddressSuggestions, setGoogleAddressSuggestions] = useState<
    GoogleAddressSuggestion[]
  >([]);
  const [isLoadingGoogleSuggestions, setIsLoadingGoogleSuggestions] =
    useState(false);
  const [days, setDays] = useState<DayDraft[]>(
    createDefaultDayDrafts(today, today),
  );
  const [existingEntries, setExistingEntries] = useState<TimeEntry[]>([]);
  const [existingRowsById, setExistingRowsById] = useState<
    Record<string, ExistingRowSnapshot>
  >({});
  const [modifiedExistingRowsById, setModifiedExistingRowsById] = useState<
    Record<string, boolean>
  >({});
  const [isLoadingExistingEntries, setIsLoadingExistingEntries] =
    useState(false);
  ...
  const companyById = useMemo(
    () =>
      Object.fromEntries(
        projectCompanies.map((company) => [Number(company.id), company]),
      ),
    [projectCompanies],
  );
      ...
      projects
        .map((project) => {
          const company = project.company_id
            ? companyById[Number(project.company_id)]
            : undefined;
          const address = [
            company?.address,
            company?.city,
  ...
  const selectedEmployeeIdsKey = selectedEmployeeIds
    .slice()
    .sort((a, b) => a - b)
    .join(",");
  ...
  useEffect(() => {
    ...
    setDays((currentDays) => {
      const nextDays = currentDays.map((day) => {
        ...
        const nextDayState: DayState =
          dayType === "worked_day"
            ...
            : dayType === "holiday"
              ? "holiday"
              : "day_off";
        const normalizedStart = normalizeTimeForInput(
          legacyEntry.clock_in ?? entry.start_time,
        );
        const normalizedEnd = normalizeTimeForInput(
          legacyEntry.clock_out ?? entry.end_time,
        );
        ...
        const fallbackEnd =
          payableHours > 0
            ? addHoursToTime(fallbackStart, payableHours + breakMinutes / 60)
            : "";
        const nextDay: DayDraft = {
          ...
          project_id:
            entry.project_id ??
            (legacyEntry.project as number | string | null) ??
            null,
          address: String(
            entry.work_location ?? meta.address ?? day.address ?? "",
          ),
          notes: String(
            legacyEntry.note ?? entry.internal_notes ?? day.notes ?? "",
          ),
  ...
  const totals = useMemo(() => {
    ...
    const estimatedGross = selectedEmployees.reduce((sum, employee) => {
      ...
      const overtimeMultiplier = Number(
        employee.overtime_rate_multiplier ?? 1.5,
      );
      return (
        sum + regularHours * rate + overtimeHours * rate * overtimeMultiplier
      );
    }, 0);
  ...
  const existingDuplicateByDate = useMemo(() => {
    ...
    for (const [key, count] of counts.entries()) {
      ...
      duplicatesByDate.set(
        date,
        Math.max(duplicatesByDate.get(date) ?? 0, count),
      );
    }
  ...
  const rowIssues = useMemo(() => {
    ...
    const addIssue = (rowId: string, message: string) => {
      ...
      if (!issuesByRow[rowId].includes(message))
        issuesByRow[rowId].push(message);
    };
    ...
    for (const day of days) {
      ...
      if (day.day_state !== "working" || day.day_type !== "worked_day")
        continue;
      ...
      if (
        day.start_time &&
        day.end_time &&
        calculateHours(day.start_time, day.end_time, day.break_minutes) <= 0
      ) {
  ...
  const updateDay = (
    ...
    setDays((current) => {
      ...
      const next = current.map((day) =>
        day.row_id === rowId ? updater(day) : day,
      );
      return next;
    });
  ...
  const deleteShiftRow = (rowId: string) => {
    ...
    if (row?.paid_day_locked) {
      notify("No se puede eliminar un día ya pagado desde aquí.", {
        type: "warning",
      });
      return;
    }
  ...
  const convertRowDayType = (rowId: string, dayType: DayDraft["day_type"]) => {
    const row = days.find((d) => d.row_id === rowId);
    if (row?.paid_day_locked) {
      notify("No se puede cambiar el tipo de un día ya pagado.", {
        type: "warning",
      });
      return;
    }
    ...
    try {
      ...
      for (const payload of payloads) {
        ...
        if (existing) {
          ...
          const unchanged =
            existingEntryComparableKey(existing) ===
            payloadComparableKey(payload);
          if (unchanged) {
            skipped += 1;
            continue;
          }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[92vh] w-[96vw] md:w-[min(96vw,1800px)] max-h-[92vh] max-w-[1800px] flex-col overflow-hidden rounded-xl border bg-background p-0 sm:!max-w-[1800px]">
        <DialogHeader className="border-b bg-background px-4 py-4 md:px-6 md:py-5">
          <DialogTitle className="text-[34px] font-semibold leading-none tracking-tight">
            Nuevo Registro
          </DialogTitle>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3 md:px-4 md:py-4">
            <div className="space-y-4">
              <div>
                    ...
                    <div className="absolute top-[calc(100%+6px)] z-40 max-h-40 w-full overflow-y-auto rounded-md border bg-background p-2 shadow-md">
                      {filteredEmployees.slice(0, 8).map((employee) => {
                        const compensationType =
                          employee.compensation_type ?? employee.pay_type;
                        return (
                          <label
                            key={employee.id}
                          ...
                          >
                            ...
                            <span className="ml-auto text-xs text-muted-foreground">
                              {compensationTypeLabels[
                                compensationType as keyof typeof compensationTypeLabels
                              ] ?? compensationType}
                              {" · "}
                ...
                {existingEntriesCountInRange > 0 &&
                !isLoadingExistingEntries ? (
                  <div className="mt-1 text-xs text-primary">
                    Se encontraron {existingEntriesCountInRange} registros
                    existentes en este rango. Puedes editarlos antes de guardar.
                  </div>
                ) : null}
              </div>
                ...
                <div className="flex items-center justify-between border-b px-3 py-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Overtime semanal de 40h (solo si el empleado tiene
                      overtime habilitado).
                    </p>
                    {hasExistingForPreviewEmployee && previewEmployee ? (
                      <p className="mt-1 text-xs text-primary">
                        Editing existing entries for:{" "}
                        <strong>{employeeOptionText(previewEmployee)}</strong>
                      </p>
                    ) : null}
                  </div>
                  <Button
                    ...
                    size="sm"
                    variant="outline"
                    onClick={applyFirstDayScheduleToAll}
                  >
                  ...
                  <table className="w-full min-w-[920px] text-sm">
                    <thead className="bg-muted/40">
                      <tr className="border-b text-left">
                        <th className="h-10 px-2 text-left align-middle font-medium whitespace-nowrap text-muted-foreground">
                          Día
                        </th>
                        <th className="h-10 w-[96px] px-2 text-left align-middle font-medium whitespace-nowrap text-muted-foreground">
                          Entrada
                        </th>
                        <th className="h-10 px-2 text-left align-middle font-medium whitespace-nowrap text-muted-foreground">
                          Lunch
                        ...
                        <th className="h-10 px-2 text-right align-middle font-medium whitespace-nowrap text-muted-foreground">
                          Horas
                        </th>
                        <th className="h-10 px-2 text-right align-middle font-medium whitespace-nowrap text-muted-foreground">
                          Pay
                      ...
                      {isLoadingExistingEntries
                        ? Array.from({ length: 5 }).map((_, index) => (
                            ...
                            >
                              ...
                              <td className="p-2">
                                <Skeleton className="ml-auto h-6 w-[48px]" />
                              </td>
                              <td className="p-2">
                        ...
                        : days.map((day, index) => {
                            ...
                            const rowDisplayHours = getPayrollHoursForDayDraft(
                              day,
                              {
                            ...
                            const addressDayNote = absentDayAddressNoteEs(
                              day.day_type,
                            );
                              ...
                              >
                                ...
                                >
                                  <Input
                                    ...
                                    disabled={
                                      isPaidRow || day.day_type !== "worked_day"
                                    }
                                    ...
                                    ? formatMoney(
                                        split.regular *
                                          ...
                                          split.overtime *
                                            getRateForPerson(previewEmployee) *
                                            Number(
                                              previewEmployee.overtime_rate_multiplier ??
                                                1.5,
                                            ),
                                      )
                                    : "—"}
                                ...
                                <td className="p-2 align-middle">
                                  <div className="relative w-[420px] pr-10">
                                    ...
                                    <Input
                                      ...
                                      value={day.address ?? ""}
                                      onFocus={() =>
                                        setActiveAddressRowId(day.row_id)
                                      }
                                      onClick={() =>
                                        setActiveAddressRowId(day.row_id)
                                      }
                                    ...
                                    {activeAddressRowId === day.row_id ? (
                                      <div className="absolute top-[calc(100%+6px)] z-40 max-h-64 w-full overflow-y-auto rounded-md border bg-background p-2 shadow-md">
                                        ...
                                        {filteredProjectAddressOptions.length >
                                        0 ? (
                                          filteredProjectAddressOptions.map(
                                            (option) => (
                                              <button
                                        ...
                                        ) : googleAddressSuggestions.length >
                                          0 ? (
                                          googleAddressSuggestions.map(
                                            (option) => (
                                              <button
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                  ...
                  {previewEmployee ? (
                    <span className="text-muted-foreground">
                      Rate:{" "}
                      <strong>
                        {formatMoney(getRateForPerson(previewEmployee))}/hr
                      </strong>
                    </span>
                  ) : null}
                  <strong>TOTAL</strong>
                  <span>
                    Horas: <strong>{footerTotals.total.toFixed(2)}</strong>
                  </span>
                  <span>
                    Regular: <strong>{footerTotals.regular.toFixed(2)}</strong>
                  </span>
                  <span>
                    OT: <strong>{footerTotals.overtime.toFixed(2)}</strong>
                  </span>
                  <span>
                    Pay: <strong>{formatMoney(footerTotals.pay)}</strong>
                  </span>
                </div>
              </div>
