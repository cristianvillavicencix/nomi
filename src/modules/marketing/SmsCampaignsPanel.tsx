...
import { appendSmsStopFooter } from "@/modules/marketing/marketingPreviewUtils";
import { SendCampaignConfirmDialog } from "@/modules/marketing/SendCampaignConfirmDialog";
import type { SmsCampaign } from "@/modules/types";
...
export const SmsCampaignsPanel = () => {
  ...
  const [detailCampaign, setDetailCampaign] = useState<SmsCampaign | null>(null);
  const [editCampaign, setEditCampaign] = useState<SmsCampaign | null>(null);
  const [sendCampaign, setSendCampaign] = useState<SmsCampaign | null>(null);

  const { data: campaigns = [], isPending } = useGetList<SmsCampaign>(
    "sms_campaigns",
    {
  ...
  const prepareMutation = useMutation({
    mutationFn: marketingCampaignActions.prepareSmsAudience,
    onSuccess: (result) => {
      notify(
  ...
  const sendMutation = useMutation({
    mutationFn: marketingCampaignActions.startSmsSend,
    onSuccess: () => {
  ...
  const cancelMutation = useMutation({
    mutationFn: marketingCampaignActions.cancelSmsCampaign,
    onSuccess: () => {
  ...
  const duplicate = (campaign: SmsCampaign) => {
    create(
      "sms_campaigns",
      {
        data: {
          ...
          include_stop_footer: campaign.include_stop_footer !== false,
        },
      },
      {
        onSuccess: () => {
  ...
  const removeDraft = (campaign: SmsCampaign) => {
    deleteOne(
      "sms_campaigns",
  ...
  if (campaigns.length === 0) {
    return (
      <p className="text-sm text-muted-foreground">
        No SMS campaigns yet. Create a draft to get started.
      </p>
    );
  }
  ...
  return (
    <>
      ...
      <SendCampaignConfirmDialog
        ...
        description="Promotional texts are sent outside the Messages inbox."
        recipientCount={sendCampaign?.recipient_count}
        isPending={sendMutation.isPending}
        preview={
          <p className="whitespace-pre-wrap text-sm">
            {sendCampaign
              ? sendCampaign.include_stop_footer !== false
                ? appendSmsStopFooter(sendCampaign.body)
                : sendCampaign.body
              : ""}
          </p>
        }
