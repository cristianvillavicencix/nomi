...
export const SubscriptionCreateReviewPanel = ({
  ...
  const paymentLabel = useMemo(() => {
    ...
    if (sendEmail && sendSms) return "Setup link · Email + SMS";
    if (sendSms) return "Setup link · SMS";
    if (sendEmail) return "Setup link · Email";
    return "Setup link";
  }, [paymentMode, cardOnFile, sendEmail, sendSms]);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col rounded-xl border bg-muted/10 p-4 md:sticky md:top-0">
      <div className="mb-3">
      ...
      {bankStatementLabel ? (
        <div className="mt-3 space-y-1.5 border-t pt-3">
          ...
          <div className="flex items-start justify-between gap-3 text-sm">
            <span className="min-w-0 font-medium uppercase leading-snug text-muted-foreground">
              {bankStatementLabel}
            </span>
          ...
          <p className="text-[11px] leading-relaxed text-muted-foreground">
            Approximate debit including processing fee. Banks may shorten or wrap
            the merchant name.
          </p>
        </div>
      ) : null}
