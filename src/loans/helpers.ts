...
export const buildReceiptNumber = (
  prefix: "ADV" | "LOAN" | "DEDUCT",
  dateIso: string,
) => {
  const stamp = (dateIso || new Date().toISOString().slice(0, 10)).replace(
    /-/g,
    "",
  );
  const randomSuffix = Math.random().toString(36).slice(2, 5).toUpperCase();
  const counterSuffix = (++receiptCounter)
    .toString(36)
    .toUpperCase()
    .padStart(2, "0");
...
export const normalizeLoanPayload = (
  rawData: LoanFormValues,
): LoanFormValues => {
  const data = { ...rawData };
  ...
  const repaymentSchedule = String(
    data.repayment_schedule ?? "next_payroll",
  ) as "next_payroll" | "specific_pay_date";
  const firstDeductionDate = String(data.first_deduction_date ?? "").trim();
  const loanDate = String(
    data.loan_date ?? new Date().toISOString().slice(0, 10),
  );
  ...
  const normalizedPaymentCount =
    recordType === "advance"
      ...
      : repaymentStrategy === "amount"
        ? Math.max(
            1,
            Math.ceil(originalAmount / Math.max(fixedInstallmentAmount, 0.01)),
          )
        : paymentCount;
  ...
  data.disbursement_receipt_number =
    String(data.disbursement_receipt_number ?? "").trim() ||
    buildReceiptNumber(recordType === "advance" ? "ADV" : "LOAN", loanDate);
...
export const getLoanStatus = (loan: Partial<EmployeeLoan>) => {
  if ((loan.remaining_balance ?? 0) <= 0 || loan.status === "completed")
    return "completed";
