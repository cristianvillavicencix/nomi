...
export const LoansToolbar = ({
  ...
  const getIdFromResponse = useCallback((data: any) => {
    ...
    if (Array.isArray(data?.data) && data.data[0]?.id != null)
        ...
        await saveContext.save?.(values, {
          ...
          mutationOptions: {
            onSuccess: (data: any) => {
              ...
              notify(
                "Saved, but I could not open the print view automatically.",
                {
                  type: "warning",
                },
              );
            ...
            onError: () => {
              notify(
                "Could not save the loan. Please review the required fields.",
                {
                  type: "error",
                },
              );
