...
export const shouldCreateDealFromWebLead = (
  settings: OrganizationLeadSettings,
  payload: { source?: string; toolKey?: string | null },
) => {
  if (!settings.auto_create_deal_from_web) return false;
  const source = payload.source?.trim();
  const toolKey = payload.toolKey?.trim();
  return Boolean(toolKey) || source === "lbs-quote";
};
