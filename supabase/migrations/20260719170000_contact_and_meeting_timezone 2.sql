-- Contact + meeting timezone for correct reminders and dual-time SMS/calendar.

alter table public.contacts
  add column if not exists timezone text;

comment on column public.contacts.timezone is
  'IANA timezone inferred from address (e.g. America/Chicago) or set manually.';

alter table public.calendar_events
  add column if not exists timezone text;

comment on column public.calendar_events.timezone is
  'IANA timezone for event_date/event_time wall clock (usually org/host booking timezone).';
