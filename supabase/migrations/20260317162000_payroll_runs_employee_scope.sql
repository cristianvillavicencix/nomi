alter table public.payroll_runs
  add column if not exists employee_id bigint references public.people(id) on update cascade on delete set null;
...
begin
  ...
  where p.status = 'active'
    and (
      ...
      or (v_run.category = 'salaried' and coalesce(p.compensation_unit, 'hour') in ('week', 'month'))
      or (v_run.category = 'subcontractor' and p.type = 'subcontractor')
      or (v_run.category = 'sales_commissions' and coalesce(p.compensation_unit, 'hour') = 'commission')
    )
  ...
  set
    remaining_balance = round(
      greatest(
        0,
        l.remaining_balance - loan_updates.total_deductions
      ),
      2
    ),
    active = case
