-- Standard users: no dollar amounts in project activity text; invoices readable only with proposals.view.
...
from (
  ...
  select
    ...
    'created change order: ' || dco.title,
    dco.created_at,
    d.org_id
...
-- Invoices: SELECT requires proposals.view (mutations already did).
drop policy if exists "client_invoices_org_scoped" on public.client_invoices;

create policy "client_invoices_select"
...
create policy "client_invoices_insert"
...
create policy "client_invoices_update"
...
create policy "client_invoices_delete"
